...
Patch Sygic GPS Navigation & Maps for Android (`com.sygic.aura`) to unlock native framerate, native render resolution, the internal debug menu, smoother navigation-camera easing, and to kill the startup "buy Premium Plus" promo popup. You might call it **Sygic Unshittifier**.
...
These are real file targets, not just aliases, so re-running `make all` is a fast no-op once the output exists — delete what you want regenerated to force a rebuild. `--all` is hardcoded here; for individual flags or a custom `-S` directory, use the scripts directly (below).
